...
from match import ORDERS, CLIENTS, INSTRUMENTS, init_orders, init_clients, init_instruments
...
class OrderBook: 
    ...
    def add_order(self, order_type, price, client_rating, quantity, client_id, arrival_time): 
        market_priority = 0 if price == "Market" else 1 
        actual_price = float('inf') if price == "Market" and order_type == "Buy" else float('-inf') if price == "Market" and order_type == "Sell" else price 
        ...
        order = (market_priority, -actual_price if order_type == "Buy" else actual_price, client_rating, arrival_time, client_id, quantity) 
    ...
    def match_order(self): 
        while self.buy_heap and self.sell_heap: 
            ...
            if -buy_order[1] >= sell_order[1]: 
                ...
                print(f"Matched {min_qty} units between {buy_order[4]} (Buy) and {sell_order[4]} (Sell)") 

                # Adjust quantities 
                if buy_order[5] > min_qty: 
                    new_buy_order = (buy_order[0], buy_order[1], buy_order[2], buy_order[3], buy_order[4], buy_order[5] - min_qty) 
                    heapq.heappush(self.buy_heap, new_buy_order) 

                if sell_order[5] > min_qty: 
                    new_sell_order = (sell_order[0], sell_order[1], sell_order[2], sell_order[3], sell_order[4], sell_order[5] - min_qty) 
